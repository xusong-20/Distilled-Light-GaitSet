...
def load_data(dataset_path, resolution, dataset, pid_num, pid_shuffle, type_equalization, cache=True):
    seq_dir = list()
    view = list()
    seq_type = list()
    label = list()
    ...
    pid_fname = osp.join('partition', '{}_{}_{}.npy'.format(
        dataset, pid_num, pid_shuffle, type_equalization))

    #
    pid_fname_teacher = osp.join('partition', '{}_{}_{}.npy'.format(
